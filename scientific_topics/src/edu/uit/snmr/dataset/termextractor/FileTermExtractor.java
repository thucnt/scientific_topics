 ...
 *
 */
public class FileTermExtractor extends TermExtractor{
	private static Logger logger = Logger.getLogger(FileTermExtractor.class);
	
	
	public TermExtractorGlobalIndexResult extractTerms(String messageSetFolder, String extractedTermsFolder) throws JATEException, IOException {
		logger.debug("Start extracting terms for corpus " + messageSetFolder);
		...
		CandidateTermExtractor wordextractor = new WordExtractor(stop, lemmatizer, false, 3);

		//Next start building candidate term and word data and index them.
		GlobalIndexBuilderMem builder = new GlobalIndexBuilderMem();
		GlobalIndexMem wordDocIndex = builder.build(new CorpusImpl(messageSetFolder), wordextractor);
		GlobalIndexMem termDocIndex = builder.build(new CorpusImpl(messageSetFolder), npextractor);
		...
		//SimpleTermExtractor tester = new SimpleTermExtractor();
		...
		execute(termDocIndex, extractedTermsFolder);



		logger.debug("Finished extracting terms for corpus " + messageSetFolder);

		return new TermExtractorGlobalIndexResult(termDocIndex, termDocFreq);
	}
